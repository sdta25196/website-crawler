...
class TProxy {
  ...
  constructor() {
    ...
    this.blacks = [] // ip
    this.proxyMaxTime = 50000
  }

  /** 添加一组数据进入 proxyPool, 添加时，顺便清理过期的 proxy */
  addProxyPool = (list) => {
    this.proxyPool = this.proxyPool.filter(proxy => this.blacks.indexOf(proxy.ip) === -1 && Date.now() - proxy.t < this.proxyMaxTime)
  ...
  getProxy = async () => {
    ...
    if (proxyItem && this.blacks.indexOf(proxyItem.ip) === -1 && Date.now() - proxyItem.t < this.proxyMaxTime) {
      return proxyItem
    } else {
